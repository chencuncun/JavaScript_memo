...
- 状态管理模式是什么？ 
  - 我们可以简单的将其看成把需要多个组件共享的变量全部存储在一个对象里面
  - 然后，将这个对象放在顶层的`Vue`实例中，让其他组件可以使用
  - 多个组件是不是就可以共享这个对象中的所有变量属性了呢？
  - `Vuex`提供了一个可以在多个组件间共享状态的插件
...
### 21.2 单界面到多界面状态管理切换
#### 21.2.1 单界面的状态管理
`State -> View -> Actions -> State`如此循环 <br/>
- `State`：状态。可以当做是`data`中的属性
- `View`：视图层。可以针对`State`的变化，显示不同的信息。
- `Actions`：主要是用户的各种操作：点击输入等，会导致状态的变化。
- 例子：计数器
  - 在`data`中定义一个`count`的属性，绑定单击按钮，点击按钮可以进行加减操作，以此改变`count`的值
#### 21.2.2 多界面的状态管理
- 多界面的状态管理
  - 多个视图都依赖同一个状态(一个状态改了，多个界面需要进行更新)
  - 不同界面的`Actions`都想修改同一个状态(`Home.vue`需要修改，`Profile.vue`也需要修改这个状态)
- 对于某些状态(状态1/状态2/状态3)来说只属于我们某一个视图，但是也有一些状态(状态a/状态b/状态c)属于多个视图共同想要维护的
  - 状态1/状态2/状态3放在自己的房间中，自己管理自己用
...
- 全局单例模式
  ...
  - 这就是`Vuex`背后的基本思想
- `State -> Vue Components -> Actions(异步操作)(Backend API:后端API) -> Mutations(Devtools插件) -> State`如此循环
...
- `Vuex`提出使用单一状态树，什么是单一状态树呢？
  - 英语：`Single Source of Truth`，也可以翻译成单一数据源
- 它是什么呢？我们来看一个生活中的例子
  - 在国内有很多信息需要被记录，比如上学时的个人档案，工作后的社保记录，公积金记录，结婚后的婚姻信息，以及其他相关的户口，医疗等信息
  - 这些信息被分散在很多地方进行管理，如果你需要办某个业务时(比如入户某个城市)，你需要到各个地点去打印，盖章资料信息，最后到一个地方提交证明你的信息无误
  - 这种保存信息的方案不仅低效，而且不方便管理，以及日后的维护也是一个庞大的工作
- 这个和我们在应用开发中比较类似
  - 如果你的状态信息是保存到多个`Store`对象中的，那么之后的管理和维护等等都会变得特别困难
  - 所以`Vuex`也使用了单一状态树来管理应用层级的全部状态
  - 单一状态树能够让我们最直接的方式找到某个状态的片段，而且在之后的维护和调试过程中，也可以非常方便的管理和维护
#### 21.4.2 `getters `的使用详解
可以看作是一个计算属性。<br/>
```
// index.js
...
const store = new Vuex.Store({
    state: {
        ...
        students: [
            ...
            { id: 110, name: '沙和尚', age: 38 },
            { id: 110, name: '唐三藏', age: 33 }
        ]
    },
    mutations: {
        // 方法
    ...
    getters: {
        ...
        // 获取年龄大于30的学生
        aboveThirtyYearStu(state) {
            // 返回年龄大于30的学生的数组
            return state.students.filter(s => s.age > 30)
        },
        // 获取年龄大于30的学生的人数
        aboveThirtyYearStuLength(state, getters) {
            return getters.aboveThirtyYearStu.length
        },
        // 输入年龄，找到数据中大于输入年龄的学生
        aboveAgeStu(state) {
            /*
...
<template>
  <div id="app">
    ...
    <p>{{ $store.getters.aboveThirtyYearStu }}</p>
    <p>{{ $store.getters.aboveThirtyYearStuLength }}</p>
    <!-- 输入年龄(比如20岁)，找到数据中大于20岁的学生并显示在网页上 -->
    <p>{{ $store.getters.aboveAgeStu(20) }}</p>
...
```
#### 21.4.3 `mutations`
- `Vuex`的`store`状态的唯一更新方式：提交`mutations`
  - 改变`state`的唯一的途径就是通过`mutations`来改变
- `mutations`主要包括两部分：
  - 字符串的事件类型(`type`)
  - 一个返回函数(`handler`)，该回调函数的第一个参数就是`state`
- `mutations`的定义方式
```
mutations: {
    increment(state){
...
export default {
  ...
  methods: {
    ...
    addStudent() {
      const stu = { id: 114, name: "蜘蛛精", age: 36 };
...
```
##### 21.4.3.2 `mutations`的提交风格
- 通过`commit`进行提交是一种普通的方式
...
mutations: {
    ...
    */
    ...
    // 第二种特殊风格下传递参数:传的第二个参数是对象
    incrementCount(state, payload) {
        state.counter += payload.count
    },
}
```
##### 21.4.3.3 `mutations`响应规则
...
- 这就要求我们必须遵守一些`Vuex`对应的规则
  ...
  - 当给`state`中的对象添加新属性时，使用下面的方式
    ...
    - 方式二：用新对象给旧对象重新赋值
- `state`中的属性都会被加入到响应式系统中，而响应式系统会监听属性的变化
...
const store = new Vuex.Store({
    ...
    mutations: {
        updateInfo(state) {
            state.info.name = '如来佛祖'

            //给info对象添加一个新的属性address，不是响应式
            // state.info['address'] = '西天'

            // 给info对象添加一个新的属性address，而且属于响应式
            // Vue.set(state.info, 'address', '西天')
...
```
##### 21.4.3.4 `mutations`的类型常量
- 在`mutations`中，我们定义了很多事件类型(也就是其中的方法名称)
- 当我们的项目增大时，`Vuex`管理的状态越来越多，需要更新状态的情况越来越多，那么意味着`mutations`中的方法越来越多
- 方法过多，使用者需要花费大量的经历去记住这些方法，甚至是多个文件间来回切换，查看方法名称，甚至如果不是复制的时候，可能会出现写错的情况
- 所以一般把事件的类型定义为一个常量
```
//mutations-type.js 
//将事件的类型定义为常量
...
import { ADDSTU } from "./store/mutations-types";
...
```
##### 21.4.3.5 `mutations`同步函数
- 通常情况下，`Vuex`要求我们`mutations`中的方法必须是同步方法
  - 主要的原因是当我们使用`devtools`时，可以帮助我们捕捉`mutations`的快照
...
devtools的显示结果：
将info中的name没有改成'如来佛祖'
```
#### 21.4.4 `actions`的使用详解
- 我们强调，不要在`mutations`中进行异步操作
  - 但是某些情况，我们确实希望在`Vuex`中进行一些异步操作，比如网络请求，必须是异步的。这个时候怎么处理呢？
  - `actions`类似与`mutations`，但是是用来代替`mutations`进行异步操作的
- 
```
// index.js 
...
```
#### 21.4.5 `modules`的使用详解
`modules`是模块的意思，为什么在`Vuex`中我们要使用模块呢？<br/>
...
const moduleA = {
    ...
    actions: {
        aUpdateName(context) {
            setTimeout(() => {
                context.commit('updateName', '铁扇公主')
            },1000)
        }
    },
    getters: {
...
export default {
  ...
  methods: {
    updateName() {
      this.$store.commit("updateName", "牛魔王");
    },
    asyncUpdateName() {
...
```
### 21.5 `store`文件夹的目录组织
- 将`state`抽离到`store`对象的上方
  - `store/index.js`
- 将`mutations`，`actions`，`getters`分别抽离到`store`文件夹中的单独的`js`文件中
  - `store/mutations.js`
  - `store/actions.js`
  - `store/getters.js`
- 在`store`文件夹中，创建`modules`文件夹，在`modules`中创建所需要的各个模块(比如以下代码的`moduleA.js`)
...
```
## 22. `axios`
### 22.1 `axios`网络请求模块的选择
`Vue`中发送网络请求有非常多的方式，那么在开发中，如何选择呢？<br/>
- 选择一： 传统的`Ajax`是基于`XMLHttpRequest(XHR)`
  - 为什么不选择它呢？
    - 配置和调试方式等非常混乱
    - 编码很麻烦
    - 所以在真实开发中很少直接使用，而是使用`jQuery-Ajax`
- 选择二：使用`jQuery-Ajax`相对于传统的`Ajax`非常好用
  - 为什么不选择它呢？
    - 首先，在`Vue`的整个开发中都是不需要使用`jQuery`了
    - 那么，就意味这为了方便我们进行一个网络请求，特意引用一个`jQuery`是不合理的
    - `jQuery`的代码`1w+`行
    - `Vue`的代码`1w+`行
...
- 选择三：官方在`Vue1.x`的时候，推出了`Vue-resource`
  ...
  - 为什么不选择它呢？
    ...
    - 那么意味着以后`Vue-resource`不再支持新的版本时，也不会再继续更新和维护
    - 对以后的项目开发和维护都存在很大的隐患
- 选择四：在说明不再继续更新和维护`Vue-resource`的同时，作者还推荐了一个框架：`axios`
  - `axios`有非常多的优点，并且用起来也非常的方便
- 在前端开发中，我们一种常见的网络请求方式是`JSONP`
...
- `JSONP`的原理是什么呢？
  ...
  - 这个时候，我们利用`<script>`标签的`src`帮助我们去服务器请求到数据，将数据当做一个`javascript`的函数来执行，
  - 并且执行的过程中传入我们需要的`json`
  - 所以，封装`jsonp`的核心就在于我们监听`window`上的`jsonp`进行回调时的名称
- 为什么选择`axios`？`axios`的功能特点是什么？
...
- URL查询对象：`params:{id:12}` (请求类型是`get`)
- 查询对象序列化函数：`paramsSerializer:function(params)`
- 请求体`request body`：`data:{key:'aa'}` (请求类型是`post`)
- 超时设置：`timeout:1000`
...
```
### 22.6 `axios`的拦截器的使用
`axios`提供了拦截器，用于我们在发送每次请求或者得到响应后，进行对应的处理 <br/>
#### 22.6.1 请求拦截
作用：<br/>
- `config`中的一些信息不符合服务器的要求
...
export function request(config) {
    ...
    instance.interceptors.request.use(config => {
        ...
        // 2.1.3 某些网络请求(比如登录token)，必须携带一些特殊的信息
        return config
    }, err => {
        // console.log(err)
    });
...
### 23.1 划分目录结构
- `src`
  - `assets`：资产：主要是用来存放图片和`css`的
    - `css`
    - `img` 
  - `common`：存放各种`js`文件：常量，混入，工具等
    - `const.js`
    - `mixin.js`
    - `utils.js`
  - `components`：各种组件
...
- `vh`：视口
- 普通的`button`或是`div`元素，都可以直接监听点击，但是组件不能直接监听点击
```
// 普通的button元素
